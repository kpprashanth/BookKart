...
<%List checkboxname= (List) request.getAttribute("checkboxname");
  Iterator itr1;String name;int ii=0;
 for(itr1=checkboxname.iterator(); itr1.hasNext();){
    
     name=(String)itr1.next();
     System.out.println("in jasp.."+name);
   %>
 checks[<%=ii%>] ="<%=name%>";     
 ...
 function check(){
  var n;
    ...
    for(n=0;n<checks.length;n++){
        if(checks[0]=="search")
        {break;}
    document.getElementById(checks[n]).checked = true; }
    }
...
function sorting(id){
    ...
    url="sortServlet";
    var count=0;var check1="",check2="",check3="";
    var selection=[];
    ...
    window.location.href="http://localhost:8080/bookshop/"+url+"?imgvalue1="+check1+"&imgvalue2="+check2+"&imgvalue3="+check3+"&count="+count;
}
...
padding: 5px;
 ...
 <body onload="check()" bgcolor="e9eaed">
		...
		<ul type="round" id="list">
			...
			<input type="checkbox" name="science fiction"/><font color="303030">Science fiction </font><br/>
	 ...
	 <div class="flip3D">
             <div class="back" style="position:absoulute; top:<%=top%>px; left:<%=left%>px;">  <a><img src="<%=s%>" id="<%=setid%>" onclick="discript(this)" width="160" height="160"></a> </div>
		<div class="front" style="position:absoulute; top:<%=top%>px; left:<%=left%>px;">  <a><img src="<%=s%>" id="<%=setid%>" onclick="discript(this)" width="160" height="160"></a>  </div>
	</div>
 ...
 <div id="signuppopup" class="pop1"> 
		...
		<a id="status" href="">Check Status</a>
                <img id="logo" src="logo2.jpg" width="80" height="90"  onclick="gohome()"/>
